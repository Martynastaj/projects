...
@section('content')
    <div class="bg-white shadow-lg rounded-lg p-6 mt-6">
        <div class="p-4 text-gray-900 mb-4">
            <h3 class="text-xl font-semibold">{{ __("LISTA KLIENTÓW") }}</h3>
        </div>

        <!-- Formularz wyszukiwania, filtrowania i sortowania -->
        <form method="GET" action="{{ route('admin.clients.searchFilter') }}" class="flex flex-wrap space-x-4 mb-6 gap-4">
            <!-- Wyszukiwanie -->
            <div class="flex-1">
                <input
                    type="text"
            ...
            <div class="flex space-x-2">
                ...
                <a href="{{ route('admin.clients.searchFilter') }}" class="bg-gray-200 text-black px-4 py-2 rounded-lg hover:bg-gray-100 transition-all">Resetuj</a>
            </div>
        </form>
            ...
            <table class="min-w-full bg-white border border-gray-300 rounded-lg shadow-md">
                ...
                <tr>
                    ...
                    <th class="py-3 px-6 text-left text-gray-600">
                        <a href="{{ route('admin.clients.searchFilter', array_merge(request()->query(), ['sortBy' => 'name', 'sortOrder' => request('sortOrder') === 'asc' ? 'desc' : 'asc'])) }}">
                            Imię
                    ...
                    <th class="py-3 px-6 text-left text-gray-600">
                        <a href="{{ route('admin.clients.searchFilter', array_merge(request()->query(), ['sortBy' => 'email', 'sortOrder' => request('sortOrder') === 'asc' ? 'desc' : 'asc'])) }}">
                            Email
                ...
                @foreach ($clients as $index => $client)
                    <tr class="border-b hover:bg-gray-50">
                        ...
                        <td class="py-3 px-6 text-sm text-gray-600">{{ $client->name }}</td>
                        <td class="py-3 px-6 text-sm text-gray-600">{{ $client->email }}</td>
                        ...
                        <td class="py-3 px-6">
                            <div class="flex space-x-2">
                                <!-- Akcja zablokuj -->
                                <form action="{{ route('admin.clients.block', $client->id) }}" method="POST" class="inline-block">
                                    @csrf
                                ...
                                <!-- Akcja aktywuj -->
                                <form action="{{ route('admin.clients.active', $client->id) }}" method="POST" class="inline-block">
                                    @csrf
                                ...
                                <!-- Akcja usuń -->
                                <form action="{{ route('admin.clients.delete', $client->id) }}" method="POST" class="inline-block ml-2" onsubmit="return confirmDeletion('{{ $client->name }}')">
                                    @csrf
                                    @method('DELETE')
                                ...
                                <script>
                                    function confirmDeletion(clientName) {
                                        return confirm('Czy na pewno chcesz usunąć użytkownika "' + clientName + '" z bazy danych?');
                                    }
                                </script>
                            </div>
                        </td>
